...
func TestRunCommand(t *testing.T) {
	...
	outStr, errStr, isTimeout, err = ssh.Run("whoami1234", 10*time.Second)
	assert.Equal(t, "", outStr)
	...
	outStr, errStr, isTimeout, err = ssh.Run("sleep 5", 1*time.Second)
	assert.Equal(t, "", outStr)
	...
	// test exit code
	outStr, errStr, isTimeout, err = ssh.Run("exit 1", 10*time.Second)
	assert.Equal(t, "", outStr)
	assert.Equal(t, "", errStr)
	assert.True(t, isTimeout)
...
func TestProxyClientSSHCommand(t *testing.T) {
	...
	outStr, errStr, isTimeout, err := ssh.Run("whoami", 10*time.Second)
	assert.Equal(t, "drone-scp\n", outStr)
	assert.Equal(t, "", errStr)
	assert.True(t, isTimeout)
	assert.NoError(t, err)
...
func TestExitCode(t *testing.T) {
	...
	outStr, errStr, isTimeout, err := ssh.Run("set -e;echo 1; mkdir a;mkdir a;echo 2", 10*time.Second)
	assert.Equal(t, "1\n", outStr)
